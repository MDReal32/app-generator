...
import { InlineConfig, ViteDevServer, createServer as viteCreateServer } from "vite";
...
const prettierConfig: PrettierOptions = JSON.parse(readFileSync("./.prettierrc", "utf-8"));
const minifyOptions = {
  removeComments: true,
  ...
  app.use("*", async (req, res) => {
    try {
      ...
      const { render } = isProd
        ...
        : await vite.ssrLoadModule(\`/src/entry-server.ts${extEnding}\`);
